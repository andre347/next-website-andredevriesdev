...
const NavLink = ({ href, children }) => {
  ...
  let className = `ml-4 px-3 py-2 font-medium text-sm leading-5 rounded-md hover:text-gray-700 focus:bg-gray-100 transition ease-in-out duration-150 ${
    router.pathname === href ? "bg-gray-100 text-gray-900" : "text-gray-500"
  }`;
  // router.pathname === href ? activeClassName : notActiveClassName;

  const theSame = router.pathname === href;
